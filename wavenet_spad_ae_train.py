...
usage: wavenet_ae_train.py [options]
...
from autoencoders.spad_ae_model import  Model
...
class _NPYDataSource(FileDataSource):
    def __init__(self, dump_root, norm=False, typ="mel", speaker_id=None, max_steps=8000,
                 cin_pad=0, hop_size=160, language = 'english'):
    ...
    def collect_files(self):
        if self.language == 'all':
            ...
            meta = join(self.dump_root, 'train.txt' + '.' + self.language)
        if not exists(meta):
...
def __train_step(device, phase, epoch, global_step, global_test_step,
                 model, optimizer, cls_optimizer, writer, criterion, cls_criterion,
    ...
    if train:
        ...
        g_hat = model.sp_train(c)
        pred = torch.argmax(g_hat,dim=1)
        cls_acc = torch.mean( (pred == g.long()).float())
        cls_loss = cls_criterion(g_hat, g.long())
        cls_loss.backward()
        cls_optimizer.step()
        writer.add_scalar("Train cls loss", float(cls_loss.item()), step)
        writer.add_scalar("Train cls acc", float(cls_acc.item()), step)
    # (B, T, 1)
    ...
    if use_cuda:
        ...
        (y_hat,cls_logits), _outputs, _replicas = data_parallel_workaround(model, (x, c, g, False))
    else:
        y_hat, cls_logits = model(x, c, g, False)

    adv_cls_pred = torch.argmax(cls_logits, dim = 1)
    adv_cls_acc = torch.mean( (adv_cls_pred == g.long()).float() )
    
    if is_mulaw_quantize(hparams.input_type):
        # wee need 4d inputs for spatial cross entropy loss
        # (B, C, T, 1)
        y_hat = y_hat.unsqueeze(-1)
        rec_loss = criterion(y_hat[:, :, :-1, :], y[:, 1:, :], mask=mask) 
        adv_cls_loss = cls_criterion(cls_logits,g.long())
        loss = rec_loss - 0.01 * adv_cls_loss
    else:
        rec_loss = criterion(y_hat[:, :, :-1, :], y[:, 1:, :], mask=mask) 
        adv_cls_loss = cls_criterion(cls_logits,g.long())
        loss = rec_loss - 0.01 * adv_cls_loss
    ...
    if train and step > 0 and step % hparams.checkpoint_interval == 0:
        ...
        save_checkpoint(device, model, optimizer, cls_optimizer, step, checkpoint_dir, epoch, ema)
    ...
    writer.add_scalar(f"{phase} adv cls loss", float(adv_cls_loss.item()), step)
    writer.add_scalar(f"{phase} adv cls acc", float(adv_cls_acc.item()), step)
    if train:
        if clip_thresh > 0:
    ...
    if train:
        return loss.item(), rec_loss.item(), cls_loss.item(), cls_acc.item(), adv_cls_loss.item() , adv_cls_acc.item()
    else:
        return loss.item(), rec_loss.item(), 0., 0., adv_cls_loss.item(), adv_cls_acc.item()


def __pretrain_ae_step(device, phase, global_step, global_test_step, global_epoch,
    ...
    y_hat = model.ae_train(x, c, g, False)
...
def __pretrain_cls_step(device, phase, 
                 model, optimizer, criterion,
                 x, y, c, g, input_lengths,
                 ):
    # x : (B, C, T)
    ...
    loss = criterion(g_hat, g.long())
    pred = torch.argmax(g_hat,dim=1)
    acc = torch.mean( (pred == g.long()).float())
    # Update
    if train:
        loss.backward()
        if clip_thresh > 0:
...
def pretrain(device, model, data_loaders, writer, ae_optimizer, cls_optimizer, checkpoint_dir = None):
    
    cls_criterion = nn.CrossEntropyLoss()
    
    if is_mulaw_quantize(hparams.input_type):
        ae_criterion = MaskedCrossEntropyLoss()
    else:
        if hparams.output_distribution == "Logistic":
            ae_criterion = DiscretizedMixturelogisticLoss()
        elif hparams.output_distribution == "Normal":
            ae_criterion = MixtureGaussianLoss()
        else:
            raise RuntimeError(
    ...
    # pretrain ae
    global global_step, global_test_step, global_epoch 
    while global_epoch  < 80 :
        for phase, data_loader in data_loaders.items():
            ...
            for step, (x, y, c, g, input_lengths) in enumerate(data_loader):
                # Do step
                ae_loss += __pretrain_ae_step(device,
                                             phase, global_step, global_test_step, global_epoch, model,
                                             ae_optimizer, ae_criterion, x, y, c, g, input_lengths,
                                             )
                if train:
                    global_step += 1
                else:
                    global_test_step +=1
            avg_ae_loss = ae_loss / len(data_loader)
            print(f"pretrain ae epo {global_epoch} {phase} avg_ae_loss {avg_ae_loss}", flush = True)    
        global_epoch +=1
        save_checkpoint(device, model, ae_optimizer, cls_optimizer, global_step, checkpoint_dir, global_epoch)
    #pretrain cls

    pre_cls_epo = 0

    while pre_cls_epo < 200:
        epo_cls_loss, epo_acc = 0., 0.
        for step, (x, y, c, g, input_lengths) in enumerate(data_loaders['train_no_dev']):
            cls_loss, acc = __pretrain_cls_step(device, 'train_no_dev', model, cls_optimizer,
                                                cls_criterion, x, y, c, g, input_lengths)
            
            epo_cls_loss += cls_loss
            epo_acc += acc

        avg_cls_loss = epo_cls_loss / len(data_loaders['train_no_dev'])
        avg_acc = epo_acc / len(data_loaders['train_no_dev'])
        print(f"pretrain cls epo {pre_cls_epo} avg_cls_loss {avg_cls_loss} acc {avg_acc}", flush = True)    
        pre_cls_epo +=1
...
def train_loop(device, model, data_loaders, optimizer, cls_optimizer, writer, checkpoint_dir=None):
    cls_criterion = nn.CrossEntropyLoss()
    if is_mulaw_quantize(hparams.input_type):
    ...
    while global_epoch < hparams.nepochs:
        for phase, data_loader in data_loaders.items():
            train = (phase == "train_no_dev")
            running_loss, rec, cls, acc, adv_cls, adv_cls_acc  = 0., 0., 0., 0., 0., 0.
            ...
            for step, (x, y, c, g, input_lengths) in enumerate(data_loader):
                ...
                # Do step
                loss = __train_step(device,
                                             phase, global_epoch, global_step, global_test_step, model,
                                             optimizer, cls_optimizer, writer, criterion, cls_criterion, x, y, c, g, input_lengths,
                                             checkpoint_dir, eval_dir, do_eval, ema)

                epo_loss, epo_rec, epo_cls, epo_cls_acc,  epo_adv_cls, epo_adv_cls_acc = loss

                running_loss += epo_loss
                rec += epo_rec
                cls += epo_cls
                acc += epo_cls_acc
                adv_cls += epo_adv_cls
                adv_cls_acc += epo_adv_cls_acc
            ...
            averaged_rec = rec / len(data_loader)
            averaged_cls = cls / len(data_loader)
            averaged_acc = acc / len(data_loader)
            averaged_adv_cls = adv_cls / len(data_loader)
            averaged_adv_cls_acc = adv_cls_acc / len(data_loader)
            #writer.add_scalar("{} loss (per epoch)".format(phase),
            #                  averaged_loss, global_epoch)
            print(f"Epo {phase} {global_epoch}  Loss: {averaged_loss} rec {averaged_rec} cls {averaged_cls}:{averaged_acc} adv_cls {averaged_adv_cls}:{averaged_adv_cls_acc}", flush = True)

        global_epoch += 1
    return ema


def save_checkpoint(device, model, ae_optimizer, cls_optimizer, step, checkpoint_dir, epoch, ema=None):
    checkpoint_path = join(
        checkpoint_dir, "checkpoint_step{:09d}.pth".format(global_step))
    ae_optimizer_state = ae_optimizer.state_dict() if hparams.save_optimizer_state else None
    cls_optimizer_state = cls_optimizer.state_dict() if hparams.save_optimizer_state else None
    global global_test_step
    torch.save({
        "state_dict": model.state_dict(),
        "ae_optimizer": ae_optimizer_state,
        "cls_optimizer": cls_optimizer_state,
        "global_step": step,
        "global_epoch": epoch,
...
def build_model():
    ...
    model = Model(wavenet = wavenet, c_in = 39, hid = 64)
    return model
...
def load_checkpoint(path, model, ae_optimizer, cls_optimizer, reset_optimizer):
    global global_step
    global global_epoch
    global global_test_step
    ...
    if not reset_optimizer:
        ae_optimizer_state = checkpoint["ae_optimizer"]
        cls_optimizer_state = checkpoint["cls_optimizer"]
        if ae_optimizer_state is not None:
    ...
    for phase in ["train_no_dev", "dev"]:
        ...
        X = FileSourceDataset(
            RawAudioDataSource(join(dump_root, phase), speaker_id=speaker_id,
                               ...
                               hop_size=audio.get_hop_size(), language = hparams.language))
        if local_conditioning:
        ...
        data_loader = data_utils.DataLoader(
            dataset, batch_size=hparams.batch_size, drop_last=drop_last,
            num_workers=hparams.num_workers, sampler=sampler, shuffle=shuffle,
            collate_fn=collate_fn, pin_memory=hparams.pin_memory)
        print(f"{phase} data loader len {len(data_loader)}",flush=True)
        speaker_ids = {}
...
if __name__ == "__main__":
    ...
    # Dataloader setup
    data_loaders = get_data_loaders(dump_root, norm=norm,typ=typ,speaker_id=speaker_id, test_shuffle=True)
    ...
    print(model)
    receptive_field = model.wavenet.receptive_field
    print("Receptive field (samples / ms): {} / {}".format(
        receptive_field, receptive_field / fs * 1000))

    from torch import optim
    ae_params = list(model.enc.parameters()) + list(model.wavenet.parameters()) + list(model.adain.parameters()) + list(model.sp_enc.parameters())

    cls_params = list(model.cls.parameters())
    Optimizer = getattr(optim, hparams.optimizer)
    ae_optimizer = Optimizer(ae_params, **hparams.optimizer_params)
    cls_optimizer = optim.Adam(cls_params, lr = 1e-3)
    if checkpoint_restore_parts is not None:
        restore_parts(checkpoint_restore_parts, model)

    # Load checkpoints
    if checkpoint_path is not None:
        load_checkpoint(checkpoint_path, model, ae_optimizer, cls_optimizer, reset_optimizer)
    ...
    pretrain(device, model, data_loaders, writer, ae_optimizer, cls_optimizer, checkpoint_dir = checkpoint_dir)


    # Train!
    ema = None
    try:
        ema = train_loop(device, model, data_loaders, ae_optimizer, cls_optimizer, writer,
        ...
        save_checkpoint(
            device, model, ae_optimizer, cls_optimizer, global_step, checkpoint_dir, global_epoch, ema)
