"""Trainining script for WaveNet VQVAE

usage: wavenet_vqvae_train.py [options]
...
def __train_step(device, phase, epoch, global_step, global_test_step,
    ...
    #    out, _outputs, _replicas = data_parallel_workaround(model, (x, c, g, False))
    #else:
    out = model(x, c, g, False)
    y_hat,vq_loss,perp = out
    vq_loss,perp = torch.mean(vq_loss), torch.mean(perp)
    if is_mulaw_quantize(hparams.input_type):
        # wee need 4d inputs for spatial cross entropy loss
        # (B, C, T, 1)
        y_hat = y_hat.unsqueeze(-1)
        loss = criterion(y_hat[:, :, :-1, :], y[:, 1:, :], mask=mask) + vq_loss
    else:
        loss = criterion(y_hat[:, :, :-1], y[:, 1:, :], mask=mask) + vq_loss
    ...
    if train:
        ...
        if clip_thresh > 0:
            ...
            #writer.add_scalar("grad norm".format(phase), grad_norm, step)
        optimizer.step()
    ...
    writer.add_scalar("{} vq loss".format(phase), float(vq_loss.item()), step)
    writer.add_scalar("{} perp ".format(phase), float(perp.item()), step)
    if train:
        if clip_thresh > 0:
            writer.add_scalar("gradient norm", grad_norm, step)
        writer.add_scalar("learning rate", current_lr, step)

    return loss.item() ,vq_loss.item(), perp.item()
...
def train_loop(device, model, data_loaders, optimizer, writer, checkpoint_dir=None):
    ...
    while global_epoch < hparams.nepochs:
        for phase, data_loader in data_loaders.items():
            train = (phase == "train_no_dev")
            running_loss, running_vq, running_perp = 0. , 0., 0.
            ...
            averaged_vq = running_vq / len(data_loader)
            averaged_perp = running_perp / len(data_loader)
            writer.add_scalar("{} loss (per epoch)".format(phase),
                              averaged_loss, global_epoch)
            print("Step {} [{}] Loss: {} vq: {} perp {}".format(
                global_step, phase, averaged_loss, averaged_vq, averaged_perp))
