...
def tau_decay_shedule(epo, model, init_tau = 1.0, anneal_rate = 0.5, anneal_interval = 25, low_thr=0.1):
    new_tau = init_tau * anneal_rate ** (epo // anneal_interval)
    if new_tau < low_thr:
        new_tau = low_thr
    #model.bottleneck.tau = new_tau
    return model, new_tau
...
def train_loop(device, model, data_loaders, optimizer, writer, checkpoint_dir=None):
    ...
    while global_epoch < hparams.nepochs:
        model, tau = tau_decay_shedule(global_epoch, model)
        for phase, data_loader in data_loaders.items():
            train = (phase == "train_no_dev")
            running_loss, running_perp = 0. , 0.
            ...
            print("Step {} [{}] Loss: {} perp: {} tau {}".format(
                global_step, phase, averaged_loss, averaged_perp, tau))
...
def build_model():
    ...
    model = CatWavAE(wavenet=wavenet,c_in=39,hid= hparams.cin_channels, tau = 1.0, k = hparams.K, frame_rate = hparams.frame_rate, hard = hparams.hard, slices = hparams.num_slices)
    return model
