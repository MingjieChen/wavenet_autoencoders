...
class NewDataset(data_utils.Dataset):

    def __init__(self, dump_root, norm=False,typ='mfcc',seg_len=32):
        ...
        stride = 16
        for path, length, sp in zip(paths, self.lengths,speaker_ids):
            for start in range(0, length-seg_len, seg_len):
                self.samples.append( (path,start,sp) )
        print(f"dataset from {self.dump_root} got {len(self.samples)} samples",flush=True)
    def __len__(self,):
        return len(self.samples)
    def __getitem__(self,i):
        path, start, sp = self.samples[i]
        feat = np.load(path)[start:start+self.seg_len]        
        return (feat.astype(np.float32).T ,sp)      
...
def new_get_data_loaders(dump_root, norm=True,typ='mfcc'):
    seg_len = hparams.max_time_steps // audio.get_hop_size()
    data_loaders = {}
    for phase in ['train_no_dev','dev']:
        is_training = phase == 'train_no_dev'

        dataset = NewDataset(dump_root = join(dump_root, phase) ,norm = norm, typ = typ,seg_len = seg_len)
...
def build_model():
    ...
    return model
"""    
def spad_train_fn(device, model, data_loaders, writer, checkpoint_dir):
    ae_opt = optim.Adam(list(model.enc.parameters()) + list(model.dec.parameters()),lr=1e-4,betas=(0.5,0.9) )
    cls_opt = optim.Adam(list(model.cls.parameters()),lr=1e-4,betas=(0.5,0.9))
    ...
    pre_ae_step = 0
    for ep in range(2):
        for step, (c,sp) in enumerate(data_loaders['train_no_dev']):
            c = c.to(device)
            #sp = sp.long().to(device)
            c_hat = model.ae_pretrain(x)
            rec_l = rec_criterion(c_hat,c)
            ...
            ae_opt.step()
            pre_ae_step += 1
            print(f"PreAE Epo {ep} Step {step} rec {rec_l.item()}",flush=True)
            writer.add_scalar(f"pre_ae", rec_l.item(),pre_ae_step)
    ...
    for ep in range(2):
        for step, (c,sp) in enumerate(data_loaders['train_no_dev']):
            ...
            sp_hat = model.sp_pretrain(c)
            cls_l = cls_criterion(sp_hat, sp)
            acc = torch.mean( (sp_hat == sp).float() )
            ...
            cls_opt.step()
            pre_cls_step += 1
            print(f"PreCls Epo {ep} Step {step} cls {cls_l.item()} acc {acc}",flush=True)


    # train
    global global_step, global_epoch, global_test_step
    for ep in range(0,hparams,nepochs):
        for step, (c,sp) in enumerate(data_loaders['train_no_dev']):
            #if global_step % 5 ==0:
...
def train_fn(device,model,data_loaders,optimizer,writer,checkpoint_dir=None):
    ...
    while global_epoch < hparams.nepochs:
        ...
        for step, (c,sp) in enumerate(data_loaders['train_no_dev']):
            ...
            running_loss += __train_step(device, global_epoch, global_step, global_test_step, model, optimizer,writer,criterion,c, checkpoint_dir)

            if global_step % hparams.test_eval_epoch_interval == 0:
                global_test_step +=1
...
def __train_step(device, epoch, global_step, global_test_step, model, optimizer, writer, criterion, c, checkpoint_dir  ):
    #is_training = (phase == 'train_no_dev')
    ...
    c = c.to(device)
    if hparams.name == 'CatAE':
        c_hat,kl = model(c)
        loss = criterion(c_hat, c) + hparams.beta * torch.mean(kl)
    else:
        c_hat = model(c)
    ...
    optimizer.step()
    if hparams.name == 'CatAE':
        print(f"Global Step [{global_step}] train loss {loss.item()} train kl {torch.mean(kl).item()}",flush=True)
    else:
        print(f"Global Step [{global_step}] train loss {loss.item()}",flush=True)
...
def eval_model(global_epoch,global_test_step, writer, device, model,criterion ,data_loader):
    ...
    for c,sp in data_loader:
        c = c.to(device)
        if hparams.name == "CatAE":
            c_hat, kl = model(c)
        else:
            c_hat = model(c)
        eval_loss += criterion(c_hat,c).item()
...
if __name__ == "__main__":
    ...
    data_loaders = new_get_data_loaders(dump_root, norm=norm,typ=typ)
